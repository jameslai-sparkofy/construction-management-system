  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/v1/projects' && method === 'POST') {
      try {
        ...
        // Insert into D1 database - handle both camelCase and snake_case
        ...
        `).bind(
          ...
          data.name || '',
          JSON.stringify(data.spcEngineering || data.spc_engineering || {}),
          JSON.stringify(data.cabinetEngineering || data.cabinet_engineering || {}),
          JSON.stringify(data.permissions || {}),
          data.createdBy || data.created_by || 'system'
        ).run();
        
        console.log('[DEBUG] Project created successfully:', projectId, result);
    ...
    // Get projects list - FROM D1
    if (path === '/api/v1/projects' && method === 'GET') {
      try {
        console.log('[DEBUG] Getting projects from D1');
        ...
        const projects = (results || []).map(project => {
          ...
          try {
            spcEng = project.spc_engineering ? JSON.parse(project.spc_engineering) : {};
          } catch (e) {
            console.error('Error parsing spc_engineering:', e);
          }
          
          try {
            cabinetEng = project.cabinet_engineering ? JSON.parse(project.cabinet_engineering) : {};
          } catch (e) {
            console.error('Error parsing cabinet_engineering:', e);
          }
    ...
    if (path === '/api/v1/auth/verify' && method === 'GET') {
      ...
      // Accept various tokens for testing
      if (token.startsWith('token-') || token === 'demo-token' || token.startsWith('test-token')) {
        return new Response(JSON.stringify({
          success: true,
          valid: true,
          user: {
