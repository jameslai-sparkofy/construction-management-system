...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const authService = new AuthService(env.SESSIONS, env.USERS, env.API_BASE_URL, env.DB_ENGINEERING);
      ...
      // Authentication routes (支援兩種路徑格式)
      // 不需要 tenantId 的路徑（預設）
      router.post('/api/v1/auth/login', (request) => 
        authRoutes.login(request)
      );
      
      router.post('/api/v1/auth/logout', (request) => 
        authRoutes.logout(request)
      );
      
      router.post('/api/v1/auth/refresh', (request) => 
        authRoutes.refresh(request)
      );
      
      router.get('/api/v1/auth/me', async (request) => {
      ...
      router.get('/api/v1/projects/:projectId', async (request) => {
        const authResult = await authMiddleware.requireAuth(request);
        if (authResult.error) return authResult.error;
      ...
      router.put('/api/v1/projects/:projectId', async (request) => {
        const authResult = await authMiddleware.requireAuth(request);
        if (authResult.error) return authResult.error;
        return projectsRoutes.updateProject(request, authResult.user, request.params.projectId);
      });

      router.delete('/api/v1/projects/:projectId', async (request) => {
        const authResult = await authMiddleware.requireAuth(request);
        if (authResult.error) return authResult.error;
      ...
      // CRM 相關路由
      router.get('/api/v1/crm/opportunities', async (request) => {
        const authResult = await authMiddleware.requireAuth(request);
        if (authResult.error) return authResult.error;
        // TODO: 實作獲取商機列表
        return ResponseHelper.success({ opportunities: [] });
      });

      router.get('/api/v1/crm/sites', async (request) => {
        const authResult = await authMiddleware.requireAuth(request);
        if (authResult.error) return authResult.error;
        // TODO: 實作獲取案場列表
        return ResponseHelper.success({ sites: [] });
      });
